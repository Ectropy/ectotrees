...
function handleMessage(session: NonNullable<ReturnType<typeof getSession>>, msg: ClientMessage) {
  const now = Date.now();

  switch (msg.type) {
    case 'ping':
      // Handled by ws library pong, but also respond at protocol level
      // (client sends JSON ping, we respond with JSON pong)
      break;
