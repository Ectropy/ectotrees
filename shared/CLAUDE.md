...
shared/
  ...
  hints.ts              # LOCATION_HINTS: array of 19 LocationHint objects ({ hint, locations }) — in-game location hints → arrays of possible exact locations, used in TreeInfoView and WorldDetailView. Also exports LOCATION_COORDS: Record<location, { x, y }> mapping each unique exact location to RS3 game coordinates (sourced from runescape.wiki/Evil_Tree), used by MapView to place pins.
