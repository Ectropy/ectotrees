...
const ALL_HINTS = LOCATION_HINTS.map(lh => lh.hint);
const ALL_LOCATIONS = [...new Set(LOCATION_HINTS.flatMap(lh => lh.locations))].sort();

type ExtraArgs = { treeHint?: string; treeExactLocation?: string };
type Args = ComponentProps<typeof WorldCard> & ExtraArgs;
...
const meta: Meta<Args> = {
  ...
  decorators: [
    (Story) => <div style={{ width: 128 }}><Story /></div>,
  ],
  argTypes: {
    treeHint: {
      control: 'select',
      options: [undefined, ...ALL_HINTS],
    },
    treeExactLocation: {
      control: 'select',
      options: [undefined, ...ALL_LOCATIONS],
    },
  },
  args: {
    world: WORLD_P2P,
    ...
    canEdit: true,
    treeHint: undefined,
    treeExactLocation: undefined,
  },
  render({ treeHint, treeExactLocation, state, ...args }) {
    return (
      <WorldCard
        {...args}
        state={{
          ...state,
          ...(treeHint !== undefined && { treeHint }),
          ...(treeExactLocation !== undefined && { treeExactLocation }),
        }}
      />
    );
  },
